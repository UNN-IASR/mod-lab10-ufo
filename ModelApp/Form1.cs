...
namespace ModelApp
{
    public partial class Form1 : Form
    {
        ...
        void draw_line(object sender, PaintEventArgs e)
        {
            ...
            g.DrawEllipse(new Pen(Color.DarkCyan, 2), (int)x1, (int)y1, 2, 2);
            g.DrawEllipse(new Pen(Color.DarkCyan, 2), (int)x2, (int)y2, 2, 2);
            GraphicsState gs;
            gs = g.Save();
            ...
            Thread.Sleep(5000);
            g.Clear(Color.White);
            g.DrawLine(new Pen(Color.DarkCyan, 2), 100, 400, 100, 0);
            g.DrawLine(new Pen(Color.DarkCyan, 2), 100, 400, 600, 400);
            for (int i = 0; i < 5; i++)
            {
                g.DrawLine(new Pen(Color.DarkCyan, 2), 90, 400 - (i + 1) * 100, 110, 400 - (i + 1) * 100);
                g.DrawLine(new Pen(Color.DarkCyan, 2), 100 + (i + 1) * 100, 390, 100 + (i + 1) * 100, 410);
            }
            g.DrawEllipse(new Pen(Color.DarkCyan, 2), 200, 10, 2, 2);
            double old_x = 200;
            double old_y = 10;
            for (int j = 1; j < 6; j++)
            {
                ...
                while (true)
                {
                    ...
                    if (Abs(x1 - x2) + Abs(y1 - y2) > error)
                    {
                        if (j != 1)
                        {
                            ...
                            old_x = 100 + 100 * j;
                            old_y = 400 - error * 100;
                            g.DrawEllipse(new Pen(Color.DarkCyan, 2), 100 + 100 * j, 400 - (float)error * 100, 2, 2);
                        }
                        break;
                    }
                    else
